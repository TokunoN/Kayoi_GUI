...
def main(page: ft.Page):
    ...
    if os.path.exists(df_filepath):
        ...
        df = pd.DataFrame(
            [[True, 5] for i in range(3)],
            columns=["run_flag", "run_span"],
            index = ["data_getting", "notify_period", "notify_alert"])
    ...
    cronsetrow_list = [data_getting, notify_alert, notify_period]


    daily_report_cb = ft.Row(controls=[
        ft.Checkbox(value=daily_report_run_flag),
        ft.Text(title_text_list[3], width = title_text_width),
        ft.Text("   "), # 見た目調整用
